...
namespace GoG.WinRT.Common
{
    public static class StorageHelper
    {
        ...
        public static void GetCredentials(string vaultName, out string userId, out string password)
        {
            userId = null;
            password = null;
            ...
            try
            {
                var creds = Vault.FindAllByResource(vaultName).FirstOrDefault();
                if (creds != null)
                {
                    userId = creds.UserName;
                    password = Vault.Retrieve(vaultName, userId).Password;
                }
            }
            catch (Exception ex)
            {
        ...
        public static void SaveCredentials(string vaultName, string userId, string password)
        {
            Vault.Add(new PasswordCredential(vaultName, userId, password));
        }

        public static void DeleteCredentials(string vaultName, string userId)
        {
            Vault.Remove(Vault.Retrieve(vaultName, userId));
        }
